...
export default function Encrypt() {
    ...
    const submitContact = async (event) => {
        ...
        let payload = {
            ...
            data: btoa(event.target.data.value),
            ip: params[0],
            jwt: params[1]
        }
        console.log(payload);
        axios.post('/api/crypto/hash', payload).then(response => {
            setHashedText(response.data.data.data);
        });
    }
    useEffect(() => {
      ...
      <>
        ...
        <div className="mt-8 flex">
            <div className="flex flex-col xl:w-1/2 lg:w-1/2 md:w-1/2 w-1/2">
                <label htmlFor="data" className="pb-2 text-sm font-bold text-gray-800 dark:text-gray-100">
                    Data
                </label>
