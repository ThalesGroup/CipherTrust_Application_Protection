...
export default function Encrypt() {
    ...
    const submitContact = async (event) => {
        ...
        let payload = {
            ...
            signedData: event.target.signed_data.value,
            signature: event.target.signature.value,
            ip: params[0],
            jwt: params[1]
        }
        console.log(payload);
        axios.post('/api/crypto/verify', payload).then(response => {
            console.log(response.data.data.verified.toString());
            setRes(response.data.data.verified.toString());
        });
    }
      ...
      <>
        ...
        <div className="mt-8 flex">
            <div className="flex flex-col xl:w-1/2 lg:w-1/2 md:w-1/2 w-1/2">
                <label htmlFor="signed_data" className="pb-2 text-sm font-bold text-gray-800 dark:text-gray-100">
                    Signed Data
                </label>
                <textarea id="signed_data" name="signed_data" required 
                ...
                placeholder="Data to be verified" 
                rows={5} 
                defaultValue={""} />
                <label htmlFor="signature" className="pb-2 text-sm font-bold text-gray-800 dark:text-gray-100">
                    Signature
                </label>
