...
void exportKeyUsage()
{
	printf ("Usage: vpkcs11_sample_find_export_key -p pin -s slotID -[k|v|c|o] keyName/opaqueObjectName [-i {k|m|u}:identifier] -w wrappingKeyName [-f format] [-e] [-m module]\n");
	printf ("-i identifier: one of 'imported key id' as 'k', MUID as 'm', or UUID as 'u'.\n");
	printf ("-k|v|c keyname to be wrapped/exported from the DSM; 'k': symmetric key name, 'v': private key, 'c': public key.");
	printf ("-o opaque object name to be wrapped/exported from the DSM");
	printf ("-w wrapping key label to be searched on the DSM.");
	printf ("-f format type: one of 'pem' or 'der' for asymmetric key output type, omit for symmetric keys.");
	printf ("-e invalidate cache and use DSM for every export request");
	exit (2);
}
