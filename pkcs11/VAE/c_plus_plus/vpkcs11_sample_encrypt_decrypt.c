/**                                                                     **
** Copyright(c) 2014 - 2019                       Confidential Material **
**                                                                      **
...
#include "vpkcs11_sample_encrypt_decrypt.h"
...
static CK_RV encryptDecrypt(char *operation, char *in_filename, char *piv, char cset_choc, char *charset, char *decrypted_file, char *encrypted_file, char *charsettype, char *header_version, int bOmitEncryption, char *tweakfilename)
{
  ...
  if (!header_version || strlen(header_version)<4 || header_version[0]!='v' || header_version[2]!='.') {
      ...
      case '5': encheader=CKM_THALES_V15HDR|CKM_VENDOR_DEFINED; if (header_version[4]=='b') encheader |= CKM_THALES_BASE64; 
                decheader=CKM_THALES_ALLHDR|CKM_VENDOR_DEFINED; if (header_version[4]=='b') decheader |= CKM_THALES_BASE64; 
                break; 
      case '1': encheader=CKM_THALES_V21HDR|CKM_VENDOR_DEFINED; 
                decheader=CKM_THALES_ALLHDR|CKM_VENDOR_DEFINED;
                break;
      case '7': encheader=CKM_THALES_V27HDR|CKM_VENDOR_DEFINED;
                decheader=CKM_THALES_ALLHDR|CKM_VENDOR_DEFINED;
                break;
      default : encheader=0; /* no header */
                decheader=0; /* no header */
                break;
  }

  {
	...
	if(!charset) {
		if (!strcmp(operation, "FPE") ) {
        	if (!strncmp(charsettype, "UTF", 3)) {
				if (tweakfilename) { FILE *pf=fopen(tweakfilename, "r"); if (pf && fread(fpeparamsutf.tweak, 1, 8, pf)==8) ; if (pf) fclose(pf); }
		...
		if (!strcmp(charsettype, "ASCII")) {
		  if (tweakfilename) { FILE *pf=fopen(tweakfilename, "r"); if (pf && fread(fpeparams.tweak, 1, 8, pf)==8) ; if (pf) fclose(pf); }
		  ...
		  plaintextlen=18*2;
            ...
            if (tweakfilename) { FILE *pf=fopen(tweakfilename, "r"); if (pf && fread(ff1paramsutf.charset+myhtonl(ff1paramsutf.charsetlen), 1, 8, pf)==8) ; if (pf) fclose(pf); ff1paramsutf.tweaklen = myhtonl(8);}
	...
	else if (!strcmp(operation, "CTR")) {pmechEncryption = &mechEncryptionCtr; pmechDecryption = &mechDecryptionCtr; }
	else if (!strcmp(operation, "GCM")) {
		pmechEncryption = &mechEncryptionGCM; pmechDecryption = &mechDecryptionGCM;
		if (aad_length > 0) {
    ...
    else if (!strcmp(operation, "ECB")) {pmechEncryption = &mechEncryptionECB; pmechDecryption = &mechDecryptionECB; }
	else if (!strcmp(operation, "RSA")) {pmechEncryption = &mechEncryptionRSA; pmechDecryption = &mechDecryptionRSA; }
    else  /* default to CBC_PAD */
	{
		pmechEncryption = &mechEncryptionPad; pmechDecryption = &mechDecryptionPad;
	}
