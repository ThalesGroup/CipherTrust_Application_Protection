...
void usage() {
    printf ("Usage: vpkcs11_sample_attributes -p pin -s slotID [-k keyName] [-kp keyPairName] [-i {k|m|u}:identifier] [-g gen_key_action] [-a alias] [-z key_size] [-ct cached_time] [-ls lifespan] [-1 customAttribute1] [-2 customAttribute2] [-3 customAttribute3] [-4 customAttribute4] [-5 customAttribute5] [-C] [-D] [-Z] [-m module]\n");
            ...
            if (hKey == CK_INVALID_HANDLE || genAction == versionRotate) {
                ...
                if (bCustomAttr == CK_TRUE) {
                    if (keyLabel) {
                        ...
                        if (rc != CKR_OK) {
                            fprintf(stderr, "Failed to create symmetric key (with custom attributes) named '%s' on DSM. \n", keyLabel);
                            break;
                        }
                    }
                }
                ...
                if (keyLabel) {
                    ...
                    if (rc == CKR_OK && hKey != CK_INVALID_HANDLE) {			
                        fprintf(stdout, "Key named '%s' successfully created on DSM. \n", keyLabel);
                        ...
                        fprintf(stderr, "Failed to create key named '%s' on DSM. \n", keyLabel);
                        break;
                    }
                }
            }
