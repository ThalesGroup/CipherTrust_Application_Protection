...
void usageCreateKey()
{
	printf ("Usage: vpkcs11_sample_create_key -p pin -s slotID -k keyName [-i {k|m|u}:identifier] [-g gen_key_action] [-a alias] [-m module] [-ls lifespan] [-ct cached_time] [-z key_size]\n");
	...
	printf ("-ct cached_time: cached time in minutes for the key");
...
#endif
{
	...
	while ((c = newgetopt(argc, argv, "k:p:m:s:g:i:as;ls:ct:z:ne;np;")) != EOF) {
		switch (c) {
		case 'p':
			pin = optarg;
			break;
		...
		if(pKsid) {
			...
			if (rc != CKR_OK)
			{
				...
				if(genAction == versionRotate)
				{
					...
					if (rc == CKR_OK && hGenKey != CK_INVALID_HANDLE)
					{
						fprintf(stdout, "Successfully rotated key with the name: '%s' on DSM! \n", keyLabel);
						getSymAttributesValue(hGenKey, 0, NULL, NULL);
					}
					else {
						fprintf(stdout, "key rotation failure, with the name: '%s' error code: %lu  on DSM! \n", keyLabel, rc);
						break;
					}
				}
				else if(genAction == versionMigrate)
				{
					...
					if(rc == CKR_OK)
					{
						fprintf(stdout, "Key with the name: %s: Migrated on DSM! \n", keyLabel);
					...
					if(!noDelete) {
						...
						if(keyLabel)	{
							if(rc == CKR_OK) {							
								fprintf(stdout, "Key with the name: %s: Deleted on DSM!! \n", keyLabel);
							}
							else {
								fprintf(stderr, "Error: Key with the name: %s: Not Deleted from DSM! rc=%lu. \n", keyLabel, rc);
								break;
							}
							...
							if (rc == CKR_OK && hGenKey != CK_INVALID_HANDLE) {					
								fprintf(stderr, "Key with name: %s created on DSM! key handle: %u, after existing key deletion. \n", keyLabel, (unsigned int)hGenKey);
								getSymAttributesValue(hGenKey, 0, NULL, NULL);
							}
							else {
								if (rc != CKR_OK) {
