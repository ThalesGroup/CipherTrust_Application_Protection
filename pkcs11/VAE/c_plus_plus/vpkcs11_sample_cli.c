 ...
 */
 ...
 #include "vpkcs11_sample_cli.h"
 #include "vpkcs11_sample_create_key.h"
 #include "vpkcs11_sample_encrypt_decrypt.h"
 #include "vpkcs11_sample_find_delete_key.h"
 #include "vpkcs11_sample_keypair_create_sign.h"
 #include "vpkcs11_sample_find_export_key.h"
 #include "vpkcs11_sample_en_decrypt_multipart.h"
 #include "vpkcs11_sample_digest.h"
 #include "vpkcs11_sample_attributes.h"
 ...
 int menu() {
   ...
   char **optArrayKeypairCreateSign[2] = {pin, key};
   ...
   printf("(1) Create/Delete a key on the DSM\n");
   ...
   switch (opSelected) {
     case 1:
      {
         ...
         if (opSelected == 1) {
           printf("\n*** Create Key ***\n");
           sampleCli("./vpkcs11_sample_create_key", optArrayCKey, 2, createKeySample);
           break;
         }
         else if (opSelected == 2) {
           printf("\n*** Delete Key ***\n");
           sampleCli("./vpkcs11_sample_find_delete_key", optArrayDKey, 2, deleteKeySample);
           break;
         }
         else if (opSelected == 3) {
           printf("\n*** Find and export key ***\n");
           sampleCli("./vpkcs11_sample_find_export_key", optArrayExportKey, 3, findExportKeySample);
           break;
         }
     }
     case 2:
     {
       ...
       if (opSelected == 1) {
         printf("\n*** Encrypt Decrypt ***\n");
         sampleCli("./vpkcs11_sample_encrypt_decrypt", optArrayEncryptDecrypt, 3, encryptDecryptSample);
         break;
       }
       else if (opSelected == 2) {
         printf("\n*** Encrypt Decrypt Multipart ***\n");
         sampleCli("./vpkcs11_sample_en_decrypt_multipart", optArrayEncryptDecryptMultipart, 2, encryptDecryptMultipartSample);
         break;
       }
     }
     case 3:
       {
         ...
         sampleCli("./vpkcs11_sample_keypair_create_sign", optArrayKeypairCreateSign, 2, keypairCreateSignSample);
         break;
       }
     case 4:
       {
         ...
         if (opSelected == 1) {
           sampleCli("./vpkcs11_sample_digest", optArrayDigest, 2, digestSample);
           break;
         }
         else if (opSelected == 2) {
           sampleCli("./vpkcs11_sample_digest", optArrayDigestHMAC, 3, digestSample);
           break;
         }
       }
     case 5:
       {
         printf("\n*** Create attribute ***\n");
         sampleCli("./vpkcs11_sample_attributes", optArrayAttributes, 2, attributesSample);
         break;
       }
     case 6:
       printf("\nExit\n");
...
int main (void)
{
	...
	printf("To better \nunderstand how the VAE API works you can \ntake a look at the sample code. \n\n");
