...
/*
   vpkcs11_sample_sign_verify.c
 */

#include "vpkcs11_sample_sign_verify.h"
...
static CK_RV signVerifyFile(char *operation, char *filename, char *signature_file, char *header_version)
{
    ...
    if (!header_version || strlen(header_version)<4 || header_version[0]!='v' || header_version[2]!='.')
      ...
      /* v2.1, v2.7 */
      case '1': headersign=CKM_THALES_V21HDR|CKM_VENDOR_DEFINED; 
                headervfy =CKM_THALES_ALLHDR|CKM_VENDOR_DEFINED; 
                break;
      case '7': headersign=CKM_THALES_V27HDR|CKM_VENDOR_DEFINED; 
                headervfy =CKM_THALES_ALLHDR|CKM_VENDOR_DEFINED; 
                break;
      default : headersign=0; /* no header */                    
                headervfy =0;
                break;
    }
...
{
    printf("Usage: vpkcs11_sample_sign_verify -p pin [-s slotID] [-g gen_key_action] [-K] [-k keyName] [-op opaqueobjectname] [-q opaquefilename] [-a alias] [-m module_path] [-o operation] [-f input_file_name] [-d signature_file_name] [-h header_version]\n");
    ...
    printf("       -K            ...create a key with well-known key bytes on the DSM (only available for SHA256-HMAC)\n");
