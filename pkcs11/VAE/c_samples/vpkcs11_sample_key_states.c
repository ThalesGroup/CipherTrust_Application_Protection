 ...
 **************************************************************************
 */
static CK_UTF8CHAR app[] = { "VORMETRIC_PKCS11_SAMPLE_KEY_STATE" };
...
void usage() {
    /*printf ("Usage: vpkcs11_sample_key_states -p pin -s slotID -k[p] keyName [-i {k|m|u}:identifier] [-ks key_state] [-m module] [ -d{c|t|a|s|d|p|r} |ps|pt key_transition_date ]\n");*/
    printf ("Usage: vpkcs11_sample_key_states -p pin -s slotID -k[p] keyName [-i {k|m|u}:identifier] [-ks key_state] [-m module]\n");
		...
		if ( pKsid ) {
            ...
            if (hKey == CK_INVALID_HANDLE) {
				...
				if (keyLabel) {
					...
					if (rc != CKR_OK) {
						fprintf(stderr, "Failed to create sym key with custom key state transition date(s) by name: %s on DSM. \n", keyLabel);
						break;
					}
					else {
						hKey = hGenKey;
