...
void usage() {
    printf ("Usage: vpkcs11_sample_attributes -p pin -s slotID [-k keyName] [-P keyPairName] [-i {k|m|u}:identifier] [-g gen_key_action] [-a alias] [-z key_size] [-ct cached_time] [-ls lifespan] [-1 customAttribute1] [-2 customAttribute2] [-3 customAttribute3] [-4 customAttribute4] [-5 customAttribute5] [-C] [-D] [-Z] [-m module]\n");
...
#endif
{
    ...
    while ((c = newgetopt(argc, argv, "p:kp:P:m:s:i:a:d:z:g:ls:ct:")) != EOF) {
        switch (c) {		
    ...
    do {
        ...
        if ( pKsid ) {
            ...
            if (hKey == CK_INVALID_HANDLE) {
				...
				if (rc != CKR_OK) {
					fprintf(stderr, "Failed to create symmetric key (with lifespan) named '%s' on DSM. \n", pKsid);
					break;
				}
                ...
                if (keyLabel) {
                    ...
                    if (rc == CKR_OK && hKey != CK_INVALID_HANDLE) {			
                        fprintf(stdout, "Key named '%s' successfully found on DSM. \n", keyLabel);					
                    }
					else {
                        fprintf(stderr, "Failed to find key named '%s' on DSM. \n", keyLabel);
                        break;
                    }
                }
            }
				...
				while(versionCnt++ < 50) {
					...
					if(clientCompromise != 0) {
						...
						if (rc != CKR_OK) {
							fprintf(stderr, "Failed to set key state to compromised with label '%s' on DSM. \n", pKsid);
							break;
						} 
						
						rc = createKey(pKsid, keyAlias, 1 /* rotate */, 0L, key_size);
						if (rc != CKR_OK) {
							fprintf(stderr, "Failed to rotate key with label '%s' on DSM. \n", pKsid);
							break;
						} 
					}
					...
					if (keyLabel) {
						rc = findKeyByLabel(keyLabel, &hKey, &objClass);
						if (rc == CKR_OK && hKey != CK_INVALID_HANDLE) {			
							fprintf(stdout, "Key named '%s' successfully found on DSM. Key handle: %u \n", keyLabel, (unsigned int)hKey);
