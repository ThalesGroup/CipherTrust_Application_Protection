...
#include "vpkcs11_sample_digest.h"
...
static CK_RV computeDigestFile(char *operation, char *filename, char *digest_file /*, char *expected*/)
{
    ...
    CK_MECHANISM	mechSHA1 = { CKM_SHA_1, NULL, 0};
    CK_MECHANISM	mechSHA1hmac = { CKM_SHA_1_HMAC, NULL, 0};
    CK_MECHANISM	mechMD5 = { CKM_MD5, NULL, 0};
    CK_MECHANISM	mechMD5hmac = { CKM_MD5_HMAC, NULL, 0};
    ...
    } else if (strcmp(operation, "SHA1") == 0) {
		pmech = &mechSHA1;
    } else if (strcmp(operation, "SHA1-HMAC") == 0) {
		pmech = &mechSHA1hmac;
    } else if (strcmp(operation, "MD5") == 0) {
		pmech = &mechMD5;
    } else if (strcmp(operation, "MD5-HMAC") == 0) {
		pmech = &mechMD5hmac;
...
void digestUsage()
{
    printf("Usage: vpkcs11_sample_digest -p pin [-s slotID] [-g gen_key_action] [-K] [-k keyName] [-a alias] [-m module_path] [-o operation] [-f input_file_name] [-d digest_file_name] [-q opaque_object_file] [-Q]\n");
    printf("       operation      ... SHA256 (default) or SHA224 or SHA384 or SHA512 or SHA1 or MD5 or MD5-HMAC\n");
	printf("                          or SHA1-HMAC or SHA224-HMAC or SHA256-HMAC or SHA384-HMAC or SHA512-HMAC\n");
    printf("       -K             ... create a key with well-known key bytes on the DSM (HMAC functions only)\n");
    printf("       -Q             ... use an opaque object previously stored on DSM without passing a binary file");
...
#endif
{
  ...
  case 'Q':
    ...
    if (strcmp(operation, "SHA256-HMAC") == 0 || strcmp(operation, "SHA224-HMAC") == 0 || strcmp(operation, "SHA512-HMAC") == 0
	|| strcmp(operation, "SHA384-HMAC") == 0 || strcmp(operation, "SHA1-HMAC") == 0 || strcmp(operation, "MD5-HMAC") == 0)
    {
      if (!use_oo)
		    use_key = 1;
    }
  ...
  // create on DSM.
  // otherwise should be searched and retrieved from DSM in the same way as
      ...
      if (rc == CKR_OK && hGenKey != CK_INVALID_HANDLE) {
        ...
        if (rc != CKR_OK) {
          ...
          printf("PASS: Successfully found and deleted %s. \n", keyLabel);
			...
			printf("Opaque object does not exist on DSM, creating...\n");
			rc = createOpaque(keyLabel, oo, oolen);
      if (rc != CKR_OK)
		    break;
  		printf("Successfully created opaque object. Now re-reading from DSM\n");
    }
    printf("Looking for opaque object with label %s\n", keyLabel);
    rc = findKey(keyLabel, keyIdLabel, CKO_THALES_OPAQUE_OBJECT, &hGenKey);
    if (CK_INVALID_HANDLE == hGenKey || rc != CKR_OK) {
        printf(" Cannot find opaque object on the DSM\n");
        break;
    }
  }
	else if (use_key) {
