...
It encrypts and decrypts the strings, and then displays the registered JCE providers.
...
1. The CADP JCE is installed. You can use the CryptoTool utility to verify the installation.
   For more information on CryptoTool, refer to the CADP JCE user guide.

2. NAE User and Key, created on Key Manager. Refer the Key Manager User Guide for instructions on 
   creating NAE Users and keys.
...
1. Modify the following parameters in build.xml file:
	...
	ingrian.home-  specify the location of CADP JCE related jar files and IngrianNAE.properties file.
 	java.location- specify the location of the Java directory.
...
3. Run the "ant dist" command to build the war file. This creates a dist directory in parallel to the src directory
   ...
   and run "ant dist" command again to build the war file:
	...
	a. Comment out the following in build.xml and place these jars in the Java classpath of Tomcat server(set CLASSPATH=<%path%> 
	in setenv.bat file of the tomcat bin folder).
...
Running the Servlet with Java Security Manager
---------------------------------------------
You can also run servlet with Java security manager. For more information on Security Manager, refer the link below:
