/*
 * CryptoSinglePartMultiThreadedC.c  
 *
...
typedef struct tagCryptoParameters
{
    int threadId;
    I_O_Session sess;
...
void usage(void)
{
    fprintf(stderr, "usage: CryptoSinglePartMultiThreadedC conf_file keyname algorithm indata iv passphrase user passwd\n"
...
#ifdef WIN32
unsigned __stdcall threadMain(void *arg2)
#else
void* threadMain(void *arg2)
#endif
{
    I_T_RETURN rc;
    CryptoParameters *cp = (CryptoParameters*)arg2;
    I_T_UINT original_encdatalen;
    int i;
    
    printf("Starting thread (%d).\n", cp->threadId);
    
    original_encdatalen = cp->encdatalen;
    ...
    for(i=0; i<100; ++i)
    {
        ...
        {
            printf("Thread (%d) successfully encrypted the data.\n", cp->threadId);
            /* print output in hex: */
            hexprint(cp->encdata, cp->encdatalen);
            cp->encdatalen = original_encdatalen;
        }
    }
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    CryptoParameters cps[10];
    ThreadHandle threadHandles[10];
    int i=0;
    I_O_CipherSpec cipherspec[NumThreads] ={0};
    I_O_Session sess; 
    ...
    if(rc != I_E_OK) {
        ...
        cps[i].indata = indata;
        cps[i].iv = iv;
        cps[i].encdata = (I_T_BYTE*)malloc(encdatalen);
    ...
    for (i = 0; i < NumThreads; i++)
    {
        Thread_CreateThread(&threadHandles[i], threadMain, (void*)&cps[i]);
    }
  
    printf("\nRun threads ...\n");
    for (i = 0; i < NumThreads; i++)
    {
        Thread_Run(threadHandles[i]);
    }
    ...
    for (i = 0; i < NumThreads; i++)
    {
        Thread_CloseHandles(threadHandles[i]);
    }
    ...
    printf("\nEnding CryptoSinglePartMultiThreadedC program with multiple threads\n");
    return 0;
}
