...
static CK_RV signAndVerifyData (unsigned mod_bits)
{
    ...
    do {
        rc = FunctionListFuncPtr->C_SignInit(
            hSession,
            &signMech,
            hKeyRSAPrivate
            );
...
void usage()
{
  printf ("Usage: vpkcs11_keypair_create_sign -p pin -s slotID -k[p] keyName [-m module]\n");
  exit (2);
}
...
int main (int argc, char* argv[])
{
	...
	while ((c = getopt(argc, argv, "p:kp:m:s:b:")) != EOF)
		switch (c) {
		case 'p':
			pin = optarg;
			break;
	...
	do
	{
		...
		else
		{
			...
			printf("Asymmetric keypair already exist.\n");
			hKeyRSAPublic = hPublicKey;
		...
		{
			printf("Successfully finding private key.\n");			
		}
		...
		{
			printf("Successfully finding public key.\n");
			rc = deleteKey(hPublicKey);
			if(rc == CKR_OK)
				printf("Successfully deleting public/private key pair.\n");
			else
				printf("Deleting public/private key pair failed.\n"); 
		}
