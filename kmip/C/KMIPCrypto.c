...
I_KS_Result Crypto(I_O_Session *handle_p, I_T_CHAR *name, I_T_BYTE *indata, I_T_BYTE *iv, unsigned int iv_len, int *keyFlag)
{
    I_KS_Result result;
    unsigned int i = 0;
    I_T_UINT count = 0;  
...
int main(int argc, char **argv)
{
    ...
    result = Crypto(&sess,keyname,indata,iv,ivlen,&keyFlag);
    if (result.status != I_KT_ResultStatus_Success)
    {
