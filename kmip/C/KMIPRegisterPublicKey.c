/*
 * KMIPRegisterPublicKey.c	 
 *
 * Sample code is provided for educational purposes
 * No warranty of any kind, either expressed or implied by fact or law
 * Use of this item is not restricted by copyright or license terms
 *
 * Sample code for KMIP Register Operation for Public Key.
 * 
 */
...
void usage(void)
{
    fprintf(stderr, "usage: KMIPRegisterPublicKey conf_file [name1] [name2]\n");
    exit(1);
}

I_KS_Result RegisterPublicKey(I_O_Session *handle_p, char *name1, char* name2)
{
...
int main(int argc, char **argv)
{
    ...
    result = RegisterPublicKey(&sess, name1, name2);
    if (result.status != I_KT_ResultStatus_Success)
    {
        printf("RegisterPublicKey failed Status:%s Reason:%s\n",
                I_KC_GetResultStatusString(result), I_KC_GetResultReasonString(result));
    }
    else
        printf("RegisterPublicKey Successful");
