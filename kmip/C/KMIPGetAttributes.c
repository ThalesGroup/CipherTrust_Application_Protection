...
void usage(void)
{
    fprintf(stderr, "usage: KMIPGetAttributes conf_file uniqueID|-name name\n");
    exit(1);
}

I_KS_Result GetAttributes(I_O_Session *handle_p, char* uniqueID_p, I_T_CHAR *name)
{
    ...
    do
    {
        ...
        result = I_KC_GetAttributes(*handle_p, attrList, NULL, 0);
        if (result.status != I_KT_ResultStatus_Success)
        {
...
int main(int argc, char **argv)
{
    ...
    result = GetAttributes(&sess, uniqueID_p, name);
    if (result.status != I_KT_ResultStatus_Success)
    {
