/*
 * KMIPDeleteAttribute.c	
 *
 * Sample code is provided for educational purposes
 * No warranty of any kind, either expressed or implied by fact or law
 * Use of this item is not restricted by copyright or license terms
 *
 * Sample code for KMIP DeleteAttribute Operation. 
 * 
 */
...
void usage(void)
{
    fprintf(stderr, "usage: KMIPDeleteAttribute conf_file uniqueID|[-name name] AttributeName Index\n");
    exit(1);
}

I_KS_Result DeleteAttribute(I_O_Session *handle_p, char* uniqueID_p, I_T_CHAR *name,char *kmipAttr,int index)
{
    ...
    do
    {
        ...
        result = I_KC_DeleteAttribute(*handle_p,attrList,kmipAttr,index);
        if (result.status != I_KT_ResultStatus_Success)
        {
            printf("I_KC_DeleteAttribute failed Status:%s Reason:%s\n",
                    I_KC_GetResultStatusString(result), I_KC_GetResultReasonString(result));
            break;
        }
        else
...
int main(int argc, char **argv)
{
    ...
    if (rc != I_E_OK)
    {
  	...
  	result = DeleteAttribute(&sess, uniqueID_p, name,attrName,index);
    if (result.status != I_KT_ResultStatus_Success)
    {
        printf("DeleteAttribute failed Status:%s Reason:%s\n",
                I_KC_GetResultStatusString(result), I_KC_GetResultReasonString(result));
    }
    else
        printf("DeleteAttribute Successful\n");
