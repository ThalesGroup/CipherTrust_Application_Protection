/*
 * KMIPModifyAttribute.c	
 *
 * Sample code is provided for educational purposes
 * No warranty of any kind, either expressed or implied by fact or law
 * Use of this item is not restricted by copyright or license terms
 *
 * Sample code for KMIP Modify Attribute Operation.
 * 
 */
...
void usage(void)
{
    fprintf(stderr, "usage: KMIPModifyAttribute conf_file uniqueID\n");
    exit(1);
}

I_KS_Result ModifyAttribute(I_O_Session *handle_p, I_T_CHAR *uniqueID_p)
{
    ...
    do
    {
        ...
        // Modify Attribute.
        result = I_KC_ModifyAttribute(*handle_p, attrList);
        if (result.status != I_KT_ResultStatus_Success)
        {
...
int main(int argc, char **argv)
{
    ...
    result = ModifyAttribute(&sess, uniqueID_p);
    if (result.status != I_KT_ResultStatus_Success)
    {
        printf("ModifyAttribute failed Status:%s Reason:%s\n",
                I_KC_GetResultStatusString(result), I_KC_GetResultReasonString(result));
    }
    else
        printf("ModifyAttribute Successful\n");
