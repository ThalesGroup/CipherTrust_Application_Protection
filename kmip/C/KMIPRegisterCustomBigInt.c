/*
 * KMIPRegisterCustomBigInt.c	
 *
 * Sample code is provided for educational purposes
 * No warranty of any kind, either expressed or implied by fact or law
 * Use of this item is not restricted by copyright or license terms
 *
 * Sample code for KMIP Register Operation with Template and Big Integer Custom Attribute.
 * 
 */
...
void usage(void)
{
    fprintf(stderr, "usage: KMIPRegisterCustomBigInt conf_file [templatename1] [templatename2]\n");
    exit(1);
}
...
I_KS_Result RegisterTemplate(I_O_Session *handle_p, char *templatename1, char* templatename2)
{
    ...
    do
    {
        ...
        //Custom Attribute : Big Number Index : 0
        bn = BN_new();
        BN_rand(bn, 64, 0, 0);
        attribute.attributeValue.index = 0;
        attribute.attributeValue.valueType_t = I_KT_AttributeValueType_Custom_S;
        attribute.attributeName = "x-Custom_BigNumber";
        attribute.attributeValue.value_u.custom_s.type = I_KT_BigInteger;
        attribute.attributeValue.value_u.custom_s.value_u.bigIntegerVal_s.byteString_p
                = malloc(BN_num_bytes(bn));
        len = BN_bn2bin(bn, attribute.attributeValue.value_u.custom_s.value_u.bigIntegerVal_s.byteString_p);
        attribute.attributeValue.value_u.custom_s.value_u.bigIntegerVal_s.byteStringLen = len;
        result = I_KC_AddToAttributeList(attrList, &attribute);
        if (result.status != I_KT_ResultStatus_Success)
        {
            printf("I_KC_AddToAttributeList failed Status:%s Reason:%s\n",
                    I_KC_GetResultStatusString(result), I_KC_GetResultReasonString(result));
            break;
        }
        BN_free(bn);
        free(attribute.attributeValue.value_u.custom_s.value_u.bigIntegerVal_s.byteString_p);
        //Custom Attribute : Big Number Index : 1
        bn = BN_new();
        BN_rand(bn, 64, 0, 0);
