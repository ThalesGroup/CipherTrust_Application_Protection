...
# MAGIC %md
# MAGIC # None Pandas UDF Benchmark
# MAGIC
# MAGIC This is the helper-first compute-cluster benchmark for the no-version (`none`) path in the new integration.
# MAGIC It is the closest counterpart to the older `internal_bulk_array_benchmark.py`,
# MAGIC but it exercises the new Python wheel and `protect_dataframe_map_in_pandas(...)` path
# MAGIC instead of Java UDF registration.
# MAGIC
# MAGIC Use this notebook when you want to answer:
# MAGIC
# MAGIC - how fast is the new helper/DataFrame path on compute cluster?
# MAGIC - how do `SPARK_GROUP_SIZE` and the v2 planner limits affect throughput?
# MAGIC - is the new helper path in the same performance range as the older bulk-array benchmark?
# MAGIC
# MAGIC Operator notes:
# MAGIC
# MAGIC - This notebook benchmarks the `mapInPandas` execution path using the installed Python wheel.
# MAGIC - It uses the installed Python wheel from the cluster library path
# MAGIC - It reads runtime config from the same copied `udfConfig.properties`
# MAGIC - It records metrics to the same style of Delta metrics table
...
SOURCE_TABLE = f"{CATALOG}.{SCHEMA}.plaintext_plaintext_none_pandas_parallelism_diag"
TARGET_TABLE = f"{CATALOG}.{SCHEMA}.plaintext_protected_none_pandas_parallelism_diag"
...
RUN_NAME = "plaintext_none_none_none_pandas_map_in_pandas_parallelism_diagnostic"
...
LOAD_PATTERN = "none_none_pandas_map_in_pandas_parallelism_diagnostic"
PROTECT_STEP_NAME = "protect_none_table_pandas_udf"
...
print(f"Generated and wrote none Pandas diagnostic source Delta table in {generate_end - generate_start:.2f} seconds.")
...
tuning_resolution = resolve_python_helper_tuning(
    ...
    v2_enable_multi_policy=EFFECTIVE_CRDP_MULTI_POLICY_ENABLED,
...
print(f"Helper DataFrame diagnostic completed in {protect_end - protect_start:.2f} seconds.")

target_count = spark.table(TARGET_TABLE).count()
print(f"Helper diagnostic protected target row count: {target_count}")
if target_count != ROW_COUNT:
    raise ValueError(
        "Helper diagnostic target row count did not match the expected source row count. "
...
revealed_df.createOrReplaceTempView("v_internal_helper_parallelism_diag_revealed")

display(spark.sql("SELECT * FROM v_internal_helper_parallelism_diag_revealed LIMIT 5"))
...
print("THALES_HELPER_DATAFRAME_DIAGNOSTIC_FINISHED")
