...
register_java_udfs(include_bulk_reveal=False)
...
# Internal + none bulk protect smoke test.
# The minimal Java surface intentionally keeps bulk protect as the tuning /
# benchmarking path. Bulk reveal was not rebuilt in this first Java phase.
...
# External bulk protect smoke test.
external_bulk_protect_df = test_df.selectExpr(
    "email_batch",
...
display(
    external_bulk_protect_df.select(
        "email_batch",
        F.expr("transform(external_email_token_batch, x -> x.protected_value)").alias("email_token_batch"),
        F.expr("transform(external_email_token_batch, x -> x.external_header)").alias("email_header_batch"),
    )
)
...
print(
    ...
    "new minimal Java UDF surface for internal, external, none, and bulk protect."
)
