...
# MAGIC %md
# MAGIC # Internal Pipeline End-to-End Bulk Load Test
# MAGIC
# MAGIC This notebook is the end-to-end internal bulk pipeline example for a
...
# MAGIC - loads those files into a Delta source table
# MAGIC - groups rows into arrays and runs object-aware bulk protect
# MAGIC
# MAGIC Use this notebook when you want:
# MAGIC
# MAGIC - realistic file-based pipeline flow
# MAGIC - ADLS landing and reload realism
# MAGIC - bulk protect instead of scalar protect for the final secure table build
# MAGIC
# MAGIC Important notes:
# MAGIC
# MAGIC - the original sample uses `custid SMALLINT`, which cannot hold large row
# MAGIC   counts
# MAGIC - this notebook widens `custid` to `BIGINT`
# MAGIC - for a low-cost first test, reduce `ROW_COUNT` before trying very large
# MAGIC   runs
# MAGIC - this is the best internal notebook when you want ADLS-backed realism plus
# MAGIC   bulk protect throughput
...
RAW_ADLS_PATH = "abfss://landing@mydatalake.dfs.core.windows.net/thales/load_test/plaintext_plaintext_bulk_raw/"
SOURCE_TABLE = f"{CATALOG}.{SCHEMA}.plaintext_plaintext_bulk_load_test"
TARGET_TABLE = f"{CATALOG}.{SCHEMA}.plaintext_protected_internal_bulk_load_test"
...
RUN_NAME = "plaintext_internal_pipeline_bulk_load_test"
...
LOAD_PATTERN = "pipeline_with_adls_bulk_object_aware"
...
spark.sql(
    f"""
    CREATE OR REPLACE TEMP VIEW v_internal_pipeline_bulk_revealed AS
    SELECT
      custid,
      name,
      thales_reveal_by_object_and_column_with_user(
...
display(spark.sql("SELECT * FROM v_internal_pipeline_bulk_revealed LIMIT 5"))
