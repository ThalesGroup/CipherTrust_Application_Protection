...
-- `compute_cluster_table_reveal_castback.sql` instead.
--
-- This older example does not implement the final cast-back pattern for
-- numeric schema columns after reveal.
--
-- The current examples also prefix session-scoped views with `temp_` to make
-- their TEMP VIEW semantics more obvious.
--
-- LEGACY ONLY:
-- - this file is retained as a reference/debug example
-- - do not use it as the recommended numeric-schema workflow
