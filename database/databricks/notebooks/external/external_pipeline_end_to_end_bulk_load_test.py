...
# MAGIC %md
# MAGIC # External Pipeline End-to-End Bulk Load Test
# MAGIC
# MAGIC This notebook is the end-to-end external bulk pipeline example for a
...
RAW_ADLS_PATH = "abfss://landing@mydatalake.dfs.core.windows.net/thales/load_test/plaintext_plaintext_external_bulk_raw/"
SOURCE_TABLE = f"{CATALOG}.{SCHEMA}.plaintext_plaintext_external_bulk_load_test"
TARGET_TABLE = f"{CATALOG}.{SCHEMA}.plaintext_protected_external_bulk_load_test"
...
RUN_NAME = "plaintext_external_pipeline_bulk_load_test"
...
LOAD_PATTERN = "pipeline_with_adls_bulk_object_aware_external"
...
flattened_df = protected_grouped_df.select(
    ...
    F.element_at(F.transform("address_struct_array", lambda x: x["protected_value"]), F.col("pos") + 1).alias("address"),
    F.element_at(F.transform("address_struct_array", lambda x: x["external_header"]), F.col("pos") + 1).alias("address_header"),
    ...
    F.element_at(F.transform("email_struct_array", lambda x: x["protected_value"]), F.col("pos") + 1).alias("email"),
    F.element_at(F.transform("email_struct_array", lambda x: x["external_header"]), F.col("pos") + 1).alias("email_header"),
    F.element_at("dob_array", F.col("pos") + 1).alias("dob"),
    F.element_at(F.transform("creditcard_struct_array", lambda x: x["protected_value"]), F.col("pos") + 1).alias("creditcard"),
    F.element_at(F.transform("creditcard_struct_array", lambda x: x["external_header"]), F.col("pos") + 1).alias("creditcard_header"),
    F.element_at(F.transform("creditcardcode_struct_array", lambda x: x["protected_value"]), F.col("pos") + 1).alias("creditcardcode"),
    F.element_at(F.transform("creditcardcode_struct_array", lambda x: x["external_header"]), F.col("pos") + 1).alias("creditcardcode_header"),
    F.element_at(F.transform("ssn_struct_array", lambda x: x["protected_value"]), F.col("pos") + 1).alias("ssn"),
    F.element_at(F.transform("ssn_struct_array", lambda x: x["external_header"]), F.col("pos") + 1).alias("ssn_header"),
)
