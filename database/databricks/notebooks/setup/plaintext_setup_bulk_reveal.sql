...
-- MAGIC
-- MAGIC DEFAULT_VOLUME_CONFIG_PATH = "/Volumes/my_catalog/my_schema/volume_forjars/config/udfConfig.properties"
-- MAGIC config_path = spark.conf.get("spark.driverEnv.UDF_CONFIG_VOLUME_PATH", None)
-- MAGIC if not config_path and Path(DEFAULT_VOLUME_CONFIG_PATH).exists():
-- MAGIC     config_path = DEFAULT_VOLUME_CONFIG_PATH
-- MAGIC     print(
-- MAGIC         "spark.driverEnv.UDF_CONFIG_VOLUME_PATH is not set; "
-- MAGIC         f"falling back to diagnostics file path {config_path}."
-- MAGIC     )
-- MAGIC     print(
-- MAGIC         "Note: Java UDF execution still requires the driver/executor runtime "
-- MAGIC         "config path to be set correctly for the jar side."
-- MAGIC     )
-- MAGIC if not config_path:
-- MAGIC     raise ValueError(
-- MAGIC         "spark.driverEnv.UDF_CONFIG_VOLUME_PATH is not set and the fallback "
-- MAGIC         "volume config path was not found. Configure the driver and executor "
-- MAGIC         "env vars before running this setup."
-- MAGIC     )
-- MAGIC
...
-- MAGIC )
-- MAGIC
-- MAGIC spark.udf.registerJavaFunction("thales_protect_by_object_and_column", "com.thales.databricks.integration.udf.ThalesProtectByObjectAndColumnUdf", T.StringType())
-- MAGIC spark.udf.registerJavaFunction("thales_protect_by_object_and_column_with_external_header", "com.thales.databricks.integration.udf.ThalesProtectByObjectAndColumnWithExternalHeaderUdf", external_protect_schema)
-- MAGIC spark.udf.registerJavaFunction("thales_reveal_by_object_and_column_with_user", "com.thales.databricks.integration.udf.ThalesRevealByObjectAndColumnWithUserUdf", T.StringType())
-- MAGIC spark.udf.registerJavaFunction("thales_reveal_by_object_and_column_with_external_header_and_user", "com.thales.databricks.integration.udf.ThalesRevealByObjectAndColumnWithExternalHeaderAndUserUdf", T.StringType())
-- MAGIC spark.udf.registerJavaFunction("thales_reveal_bulk_by_object_and_column_with_user", "com.thales.databricks.integration.udf.ThalesRevealBulkByObjectAndColumnWithUserUdf", T.ArrayType(T.StringType()))
-- MAGIC spark.udf.registerJavaFunction("thales_reveal_bulk_by_object_and_column_with_external_header_and_user", "com.thales.databricks.integration.udf.ThalesRevealBulkByObjectAndColumnWithExternalHeaderAndUserUdf", T.ArrayType(T.StringType()))
-- MAGIC
-- MAGIC print("Required object-aware Thales Java UDFs registered for plaintext bulk reveal setup.")
...
-- MAGIC INTERNAL_OBJECT = f"{CATALOG}.{SCHEMA}.plaintext_protected_internal"
-- MAGIC EXTERNAL_OBJECT = f"{CATALOG}.{SCHEMA}.plaintext_protected_external"
-- MAGIC NONE_OBJECT = f"{CATALOG}.{SCHEMA}.plaintext_protected_none"
-- MAGIC INTERNAL_ARRAY_OBJECT = f"{CATALOG}.{SCHEMA}.plaintext_protected_internal_arrays"
-- MAGIC EXTERNAL_ARRAY_OBJECT = f"{CATALOG}.{SCHEMA}.plaintext_protected_external_arrays"
-- MAGIC NONE_ARRAY_OBJECT = f"{CATALOG}.{SCHEMA}.plaintext_protected_none_arrays"
-- MAGIC
-- MAGIC def _load_properties(path_str):
-- MAGIC     props = {}
-- MAGIC     for raw_line in Path(path_str).read_text(encoding="utf-8").splitlines():
-- MAGIC         line = raw_line.strip()
-- MAGIC         if not line or line.startswith("#") or "=" not in line:
-- MAGIC             continue
-- MAGIC         key, value = line.split("=", 1)
-- MAGIC         props[key.strip()] = value.strip()
-- MAGIC     return props
-- MAGIC
-- MAGIC props = _load_properties(config_path)
-- MAGIC
-- MAGIC print("Plaintext setup runtime diagnostics:")
-- MAGIC print(
-- MAGIC     {
-- MAGIC         "config_path": config_path,
-- MAGIC         "current_catalog": spark.sql("SELECT current_catalog()").first()[0],
-- MAGIC         "current_schema": spark.sql("SELECT current_schema()").first()[0],
-- MAGIC         "runtime_api_version": props.get("CRDP_API_VERSION", "v2"),
-- MAGIC         "runtime_transport_mode": props.get("CRDP_TRANSPORT_MODE", "auto"),
-- MAGIC         "runtime_crdp_ip": props.get("CRDPIP", "<not set>"),
-- MAGIC         "runtime_crdp_port": props.get("CRDPPORT", "<not set>"),
-- MAGIC         "runtime_batch_size": props.get("BATCH_SIZE", "<not set>"),
-- MAGIC         "runtime_request_item_target": props.get("CRDP_REQUEST_ITEM_TARGET", "<not set>"),
-- MAGIC         "runtime_spark_group_size": props.get("SPARK_GROUP_SIZE", "<not set>"),
-- MAGIC     }
-- MAGIC )
-- MAGIC
-- MAGIC print("Object mapping diagnostics:")
-- MAGIC for object_name in [
-- MAGIC     INTERNAL_OBJECT,
-- MAGIC     EXTERNAL_OBJECT,
-- MAGIC     NONE_OBJECT,
-- MAGIC     INTERNAL_ARRAY_OBJECT,
-- MAGIC     EXTERNAL_ARRAY_OBJECT,
-- MAGIC     NONE_ARRAY_OBJECT,
-- MAGIC ]:
-- MAGIC     mapping_key = f"protect.object.{object_name}"
-- MAGIC     print(
-- MAGIC         {
-- MAGIC             "object_name": object_name,
-- MAGIC             "mapping_key": mapping_key,
-- MAGIC             "mapping_value": props.get(mapping_key, "<not found>"),
-- MAGIC         }
-- MAGIC     )
-- MAGIC
-- MAGIC print("Profile alias diagnostics:")
-- MAGIC for alias_key in [
-- MAGIC     "TAG.char.internal",
-- MAGIC     "TAG.nbr.internal",
-- MAGIC     "TAG.char.external",
-- MAGIC     "TAG.nbr.external",
-- MAGIC     "TAG.char.none",
-- MAGIC     "TAG.nbr.none",
-- MAGIC ]:
-- MAGIC     print({alias_key: props.get(alias_key, "<not found>")})

-- COMMAND ----------

CREATE OR REPLACE TABLE plaintext_plaintext (
  custid SMALLINT,
  name VARCHAR(107),
  address VARCHAR(107),
  city VARCHAR(107),
...
INSERT OVERWRITE plaintext_plaintext
SELECT * FROM VALUES
...
CREATE OR REPLACE TABLE plaintext_protected_internal (
  custid SMALLINT,
  name VARCHAR(107),
  address STRING,
  city VARCHAR(107),
...
INSERT OVERWRITE plaintext_protected_internal
SELECT
  custid,
  name,
...
SELECT *
FROM plaintext_protected_internal
ORDER BY custid;
...
CREATE OR REPLACE TABLE plaintext_protected_external (
  custid SMALLINT,
  name VARCHAR(107),
  address STRING,
...
CREATE OR REPLACE TABLE plaintext_protected_none (
  custid SMALLINT,
  name VARCHAR(107),
  address STRING,
  city VARCHAR(107),
...
INSERT OVERWRITE plaintext_protected_external
SELECT
  custid,
  name,
...
SELECT *
FROM plaintext_protected_external
ORDER BY custid;
...
INSERT OVERWRITE plaintext_protected_none
SELECT
  custid,
  name,
...
CREATE OR REPLACE TABLE plaintext_protected_internal_arrays
USING DELTA AS
...
SELECT *
FROM plaintext_protected_internal_arrays
ORDER BY batch_id;
...
CREATE OR REPLACE TABLE plaintext_protected_external_arrays
USING DELTA AS
WITH ordered_rows AS (
  SELECT
    *,
...
SELECT *
FROM plaintext_protected_external_arrays
ORDER BY batch_id;
...
CREATE OR REPLACE TABLE plaintext_protected_none_arrays
USING DELTA AS
WITH ordered_rows AS (
  SELECT
    *,
...
SELECT *
FROM plaintext_protected_none_arrays
ORDER BY batch_id;

-- COMMAND ----------

-- Internal array reveal view using bulk reveal directly on the protected array table
...
-- External array reveal view using bulk reveal directly on the protected array table
...
-- None array reveal view using bulk reveal directly on the protected array table
...
-- MAGIC %python
-- MAGIC current_catalog_name = spark.sql("SELECT current_catalog()").first()[0]
-- MAGIC current_schema_name = spark.sql("SELECT current_schema()").first()[0]
-- MAGIC fq_schema = f"{current_catalog_name}.{current_schema_name}"
-- MAGIC grant_statements = [
-- MAGIC     f"GRANT USE CATALOG ON CATALOG {current_catalog_name} TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT USE SCHEMA ON SCHEMA {fq_schema} TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_plaintext TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_protected_internal TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_protected_external TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_protected_none TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_protected_internal_arrays TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_protected_external_arrays TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT SELECT ON TABLE {fq_schema}.plaintext_protected_none_arrays TO `thales_cluster_deployers`",
-- MAGIC     f"GRANT USE CATALOG ON CATALOG {current_catalog_name} TO `analyst`",
-- MAGIC     f"GRANT USE SCHEMA ON SCHEMA {fq_schema} TO `analyst`",
-- MAGIC ]
-- MAGIC
...
-- Validation checks
SELECT 'base' AS object_type, COUNT(*) AS row_count FROM plaintext_plaintext
UNION ALL
SELECT 'internal_table' AS object_type, COUNT(*) AS row_count FROM plaintext_protected_internal
UNION ALL
SELECT 'external_table' AS object_type, COUNT(*) AS row_count FROM plaintext_protected_external
UNION ALL
SELECT 'none_table' AS object_type, COUNT(*) AS row_count FROM plaintext_protected_none
UNION ALL
