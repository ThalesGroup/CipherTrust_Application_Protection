...
# MAGIC %md
# MAGIC # Legacy Internal Debug Reveal Examples
# MAGIC
# MAGIC DEBUG ONLY:
# MAGIC - use `numbers_reveal_castback_examples.py` or
# MAGIC   `numbers_reveal_castback_examples.sql` for the current supported flow
# MAGIC - this notebook is for troubleshooting reveal behavior, raw outputs, and batch wiring
# MAGIC
# MAGIC This notebook is aligned to `legacy_secure_view_examples.sql`.
# MAGIC It registers the Java UDFs, recreates the legacy/debug reveal views, builds the
# MAGIC batched array table, and runs diagnostic queries against the objects in that flow.
# MAGIC
# MAGIC Use this only for troubleshooting legacy reveal behavior, not as the primary
# MAGIC customer example.
# MAGIC
# MAGIC TEMP VIEW naming note:
# MAGIC - the session-scoped views in this notebook are prefixed with `temp_`
# MAGIC - that makes it clearer these are compute-cluster testing objects, not persistent UC views
# MAGIC
...
# NOTE:
# Use `numbers_reveal_castback_examples.py` or
# `numbers_reveal_castback_examples.sql` for the current
# supported numeric-schema flow. The older non-castback
# `legacy_secure_view_examples.sql` is retained only as a
# legacy/debug artifact.
...
ROW_REVEAL_VIEW = "temp_v_plaintext_protected_internal_reveal"
ARRAY_REVEAL_VIEW = "temp_v_plaintext_protected_internal_array_reveal"
FINAL_FLAT_VIEW = "temp_v_plaintext_final_reveal_flat"
BATCH_SIZE = 1000
