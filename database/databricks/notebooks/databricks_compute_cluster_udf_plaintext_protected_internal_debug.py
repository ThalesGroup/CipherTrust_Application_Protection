...
# MAGIC %md
# MAGIC # Thales CRDP Compute Cluster Debug Notebook - plaintext_protected_internal
# MAGIC
# MAGIC DEBUG ONLY:
# MAGIC - use `compute_cluster_table_reveal_castback.py` or
# MAGIC   `compute_cluster_table_reveal_castback.sql` for the current supported flow
# MAGIC - this notebook is for troubleshooting reveal behavior, raw outputs, and batch wiring
# MAGIC
# MAGIC This notebook is aligned to `legacy_compute_cluster_secure_view_examples.sql`.
# MAGIC It registers the Java UDFs, recreates the legacy/debug reveal views, builds the
# MAGIC batched array table, and runs a few diagnostic queries against the objects
# MAGIC in that flow.
# MAGIC
...
# NOTE:
# Use `compute_cluster_table_reveal_castback.py` or
# `compute_cluster_table_reveal_castback.sql` for the current
# supported numeric-schema flow. The older non-castback
# `legacy_compute_cluster_secure_view_examples.sql` is retained only as a
