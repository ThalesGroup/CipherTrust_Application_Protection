...
//  @Override
    ...
    static {
        try {
            String volumePath = System.getenv("UDF_CONFIG_VOLUME_PATH"); // Environment variable

            if (volumePath == null || volumePath.isEmpty()) {
                throw new RuntimeException("Environment variable UDF_CONFIG_VOLUME_PATH is not set.");
            }

            Path path = Paths.get(volumePath);
            if (Files.notExists(path)) {
                throw new RuntimeException("Unable to find udfConfig.properties at: " + volumePath);
            }
            ...
            throw new RuntimeException("Error loading properties file from volume using environment variable.", ex);
        }
    }
