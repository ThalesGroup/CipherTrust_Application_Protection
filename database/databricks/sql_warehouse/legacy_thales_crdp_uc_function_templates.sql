-- LEGACY: Thales CRDP v1 Unity Catalog SQL Warehouse function templates
--
-- Replace:
--   <catalog>
--   <schema>
...
--   <PROTECTION_PROFILE>
--   <UC_WHEEL_PATH>
--   COLUMN_PROFILES and PROFILE_ALIASES
--
-- Recommended:
-- - Mark reveal functions NOT DETERMINISTIC
-- - Use *_with_user functions only behind SQL views or wrappers that inject
--   session_user() or current_user()
-- - Do not expose reveal_user as a free-form BI parameter

CREATE OR REPLACE FUNCTION <catalog>.<schema>.thales_crdp_scalar_legacy(
  value STRING,
  mode STRING,
  datatype STRING
)
...
KEYMETADATA = "<DEFAULT_METADATA>"
REVEAL_USER = "<DEFAULT_REVEAL_USER>"
...
CREATE OR REPLACE FUNCTION <catalog>.<schema>.thales_crdp_scalar_by_column(
  value STRING,
  mode STRING,
  datatype STRING,
  column_name STRING
...
DEFAULT_REVEAL_USER = "<DEFAULT_REVEAL_USER>"
RETURN_CIPHERTEXT = True
BADDATATAG = "99999999999"
...
CREATE OR REPLACE FUNCTION <catalog>.<schema>.thales_crdp_scalar_by_column_with_user(
  value STRING,
  mode STRING,
  datatype STRING,
  column_name STRING,
...
normalized = check_valid(value, datatype)
policy_name, policy_type = resolve_profile(datatype, column_name)
effective_reveal_user = (reveal_user or "").strip() or "<DEFAULT_REVEAL_USER>"
...
CREATE OR REPLACE FUNCTION <catalog>.<schema>.thales_crdp_bulk_legacy(
  values ARRAY<STRING>,
  mode STRING,
  datatype STRING
)
...
CREATE OR REPLACE FUNCTION <catalog>.<schema>.thales_crdp_bulk_by_column(
  values ARRAY<STRING>,
  mode STRING,
  datatype STRING,
  column_name STRING
...
CREATE OR REPLACE FUNCTION <catalog>.<schema>.thales_crdp_bulk_by_column_with_user(
  values ARRAY<STRING>,
  mode STRING,
  datatype STRING,
  column_name STRING,
...
-- Recommended secure wrappers:
-- Use these patterns in views, notebooks, or governed SQL objects so the
-- session identity is supplied by Databricks instead of end users.

-- Example scalar secure reveal:
-- SELECT <catalog>.<schema>.thales_crdp_scalar_by_column_with_user(
--   token_col, 'reveal', 'char', 'email', session_user()
-- ) AS email_cleartext
-- FROM my_table;

-- Example bulk secure reveal:
-- SELECT <catalog>.<schema>.thales_crdp_bulk_by_column_with_user(
--   token_array_col, 'revealbulk', 'char', 'email', session_user()
-- ) AS email_cleartext_array
-- FROM my_table;
