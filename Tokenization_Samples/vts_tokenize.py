...
#################################################################################
#                                                                               #
# Tokenize a given plaintext using the token group and token template specified.#
# The token group and token template must already accessible to the server and  #
# operational. If the operation succeeds the token is returned or, if a failure #
# occurred, an error will be returned.                                          #
#                                                                               #
...
def usage(msg=None):
    ...
    print("\t-l\t\tURL of VTS")
    print("\t-p\t\tThe original text user data to be tokenize.")
    print("\t-I\t\tPlain text file name original user data to be tokenize.")
...
if not p11ptext:
    if not p11input:
        p11input = "-"
    if p11input == "-":
        p11ptext = sys.stdin.read()
    else:
        st = os.stat(p11input)
        ifd = open(p11input, "r")
        tempText = ifd.read(st.st_size)
        ifd.close()
        print(tempText.strip("\n"))
        p11ptext = base64.b64encode((tempText.strip("\n").encode("utf-8")))
        p11ptext = p11ptext.decode("utf-8")
